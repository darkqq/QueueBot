...
from keyboards import get_lessons, convert_to_bsuir_week
from dateutil.parser import parse
import datetime
import calendar


def create_callback_data(action, year, month, day, week="-1"):
    """ Create the callback data associated to each button"""
    return ";".join([action, str(year), str(month), str(day), str(week)])
...
def process_calendar_selection(bot, query):
    ...
    ret_data = (bot.answer_callback_query(callback_query_id=query.id),
                False, None)
    (action, year, month, day, week) = separate_callback_data(query.data)
    ...
    if action == "IGNORE":
        ...
        bsuir_week = convert_to_bsuir_week(week)
        response = bot.edit_message_text(text=f'{day}.{month}({bsuir_week} неделя) Выберите предмет:',
                                         chat_id=query.message.chat.id,
                                         message_id=query.message.message_id,
                                         reply_markup=get_lessons(921703, date.weekday(), bsuir_week)
                                         )
        ret_data = response, True, datetime.datetime(int(year), int(month), int(day)), week
